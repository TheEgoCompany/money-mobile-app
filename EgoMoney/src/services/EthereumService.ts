  ...
  async createWallet(): Promise<HDNodeWallet> {
    return new Promise((resolve, reject) => {
      try {
        ...
        reject(new Error('Failed to create wallet: ' + (error as Error).message));
      }
    });
  }
...
const ethService = new EthereumService(
  process.env.PUBLIC_ALCHEMY_ETH_KEY,
  process.env.PUBLIC_ALCHEMY_ETH_URL,
  process.env.PUBLIC_ALCHEMY_SOCKET_URL,
